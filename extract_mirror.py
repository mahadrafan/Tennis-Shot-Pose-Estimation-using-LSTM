...
def main():
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if missing > SEQ_LENGTH * 0.3:
                    print(f"  [skip] too many frames without pose ({missing}/{SEQ_LENGTH}). try in another moment.")
                    continue
    ...
    if not annotations:
        print("No annotation. exit to save.")
        return

    print(f"\nTotal annotation: {len(annotations)}")
    print("saving to CSV...")
